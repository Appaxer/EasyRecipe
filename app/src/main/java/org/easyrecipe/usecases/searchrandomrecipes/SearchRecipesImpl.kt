...
class SearchRecipesImpl @Inject constructor(
    ...
    override suspend fun execute(request: SearchRecipes.Request) = runUseCase {
        val recipes = recipeRepository.getRemoteRecipes(request.name, request.mealType)
        SearchRecipes.Response(recipes)
    }
}
